...
**Canonical repository: `Ingwarski/NanoDuck-AI-Consulting`.** Imported from Neo at `a6f5d47303a7fa785a588e036ad39636b99a5dff`, retaining its source history, approved Electric A v8 design and saved model settings. See the [import record](IMPORT.md) for verification and inherited SDD gaps. The Northflank/Cloudflare configuration below describes Neo's existing deployment; importing it does not connect this repository to live infrastructure or authorize a release.
...
Google does not accept Northflank's generated `*.code.run` hostname as an OAuth redirect domain. The deployment therefore uses the repository's small Cloudflare Worker as its stable public HTTPS origin, and NanoDuck requires the Worker's shared secret header on every non-health request. Northflank remains the application and database runtime; the Worker stores no application data. The runbook contains the exact endpoint and release order.
