...
for (const field of ['name', 'version', 'main']) {
  if (typeof packageMetadata[field] !== 'string' || !packageMetadata[field].trim()) errors.push(`package.json: application requires a non-empty ${field} field`);
}
...
if (typeof packageMetadata.main !== 'string' || !packageMetadata.main.trim()) errors.push('package.json: application requires a non-empty main entry');
else {
