...
test("preflight inspects a synthetic local subscription catalog without a model turn", async () => {
  ...
  try {
    ...
    const result = await run(["src/server/preflight.mjs"], {
      ...processEnvironment(), NODE_ENV: "test", NANODUCK_DATA_DIR: `${directory}/data`, CODEX_HOME: directory,
    ...
    const absent = await run(["src/server/preflight.mjs"], { ...processEnvironment(), NODE_ENV: "test", NANODUCK_DATA_DIR: `${directory}/data` });
