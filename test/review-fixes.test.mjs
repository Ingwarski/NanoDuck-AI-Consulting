...
test('checkpointed follow-up evidence and replies survive an encrypted restart without replay', async t => {
  const { mkdtemp, rm } = await import('node:fs/promises'); const { tmpdir } = await import('node:os'); const { join } = await import('node:path'); const { randomBytes } = await import('node:crypto');
  const { createLocalStore } = await import('../src/server/local-store.mjs');
  const dataDirectory = await mkdtemp(join(tmpdir(), 'nanoduck-review-')); const dataKey = randomBytes(32);
