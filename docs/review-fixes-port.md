...
## Additional recurring-pause repair: source metadata storage

Check `src/server/recovery.mjs` (and the target's SQL columns/validators) against both provider adapters. The adapters preserve full source titles/claims: legacy `cleanText(value, 280/1000)` calls no longer enforce those historical lengths. A valid claim over 1000 characters therefore passes inference but fails `validateLocalState` through recovery validation, discarding a completed answer and pausing the run. Do not reinstate arbitrary truncation. Align source title/claim validation with the existing stored-message field ceiling (16 MiB in the local implementation) and retain whole-state/envelope, URL, language and encryption protections. Adapt a narrower target SQL schema through its normal migration mechanism before activating; do not merely weaken a validator while leaving the column too small.

In `consultation.mjs`, categorize `local_state_invalid` and `local_store_capacity_exceeded` explicitly and show their actual recovery action, rather than only `unexpected_failure`. Never log the rejected answer or credentials. Add an actual adapter-to-durable-store test using synthetic source title >280 and claim >1000, then complete a consultation, reopen encrypted storage and backup/restore it; all text must survive exactly. The reference regression is `test/source-persistence.test.mjs` with a deterministic `fake-codex.mjs` source fixture.

A Git push alone does not update an already running Node process. Activate the corrected server only after verifying zero active runs; preserve an encrypted backup and existing credentials. Verify CA-trusted HTTPS health and use authenticated Retry to resume the missing checkpoint, never resubmit the full owner question or rerun successful consultants. If the historic discarded response and exact exception were not logged, distinguish the reproduced storage defect from a proven reconstruction of that specific answer.
