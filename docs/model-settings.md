...
The current source pins `@openai/codex` to `0.153.1` and `@anthropic-ai/claude-code` to `2.1.258`. Codex exposes only its live inspected catalog. The owner-supplied Claude desktop picker screenshot dated 18 September 2026 is the source for the Claude Code Settings vocabulary: model **Opus 5** (`claude-opus-5`) and effort choices **Low**, **Medium**, **High** (the default), **Extra**, and **Max**. The app exposes that verified model plus any owner-configured candidate IDs only after its content-free managed-authentication check. It does not expose a fictitious “Claude Code default” model or unsupported `default`/`xhigh` effort values. At the isolated Claude CLI boundary only, `claude-opus-5` maps to the installed CLI's `opus`, and the visible `Extra` maps to its `xhigh` argument; snapshots, Settings and exported records retain the exact owner-facing Opus 5 / Extra vocabulary. The inactive Claude branch remains absent from saved preferences until the owner selects and saves it. The rejected initial prototype offered only the historical Astra example. The three revision-2 candidates now demonstrate selection using a copied, read-only local desktop Codex `model/list` catalog observed on 13 September: see [catalog evidence](../forge/design/evidence/local-model-catalog-20260913.json). This is local discovery evidence, not proof of the deployed app catalog or execution entitlement. It does not change the current saved selections. Production must continue to inspect the selected provider before accepting a model/effort combination.
