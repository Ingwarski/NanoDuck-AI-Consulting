...
The current source pins `@openai/codex` to `0.153.1` and `@anthropic-ai/claude-code` to `2.1.258`. It dynamically discovers capabilities. Do not replace that discovery with a speculative hard-coded list. The prototype intentionally offers only the recorded Astra example; its controls do not constitute a provider capability catalog.
