...
The current source pins `@openai/codex` to `0.153.1` and `@anthropic-ai/claude-code` to `2.1.258`. Codex exposes only its live inspected catalog. Claude Code exposes the provider-default entry plus only owner-configured candidate IDs after its content-free managed-authentication check; it never invents a named Claude model from a historical record. The inactive Claude branch remains absent from saved preferences until the owner selects and saves it. The rejected initial prototype offered only the historical Astra example. The three revision-2 candidates now demonstrate selection using a copied, read-only local desktop Codex `model/list` catalog observed on 13 September: see [catalog evidence](../forge/design/evidence/local-model-catalog-20260913.json). This is local discovery evidence, not proof of the deployed app catalog or execution entitlement. It does not change the current saved selections. Production must continue to inspect the selected provider before accepting a model/effort combination.
