...
18 September 2026 · Revision 5 · Authority: current [product-idea.md](product-idea.md) · Working language: English

## Product identity

Use **NanoDuck Consulting Group** as the app name, following the product idea. The SVG brand mark and palette belong to the design brief. Existing consultant behavior and IDs are retained; access and data boundaries are reconciled for local and same-network operation.
...
Success path: 1. Owner submits text with optional supported attachments. 2. System confirms accepted work. 3. Head inventories the complete owner request and later corrections, chooses the actual specialist roles within the selected team size, and gives each a distinct task whose exact wording is delivered to that recipient. 4. Each specialist gives an independent initial position to Critic. Critic reviews every specialist and can direct a specific stop-and-rework when a reply is circular, irrelevant, fabricated or unsupported; the consultant corrects it, gives a grounded objection or acknowledges unavailable evidence. Head reviews the whole team after each round and decides whether another targeted round is useful, subject to the owner-selected ceiling. Specialists do not message each other. 5. Each specialist submits its final position to Head; Critic reviews all final positions together and sends its closing assessment to Head. 6. Only then Head delivers Consolidated advice covering all requested outputs and source links, naming any missing item and unresolved disagreement.
...
| FR-02.3 | Every accepted question uses the owner-selected number of relevant permitted specialists: 1, 2, 3, 5, or a 1–5 count selected by Head in Auto. Head selects the actual roles in both fixed and Auto modes; the application server validates the roster and executes Head's routing. The count excludes Head and Critic. Leadership Consultant and esoteric roles are excluded. Spiritual Consultant applies the specified evangelical Protestant doctrine; Psychotherapist may use major classical psychotherapy schools and Internal Family Systems within the stated non-clinical/emergency boundary. | UC-002 |
| FR-02.4 | Every participating consultant and Critic is a separate ephemeral model invocation reconstructed from the saved event stream, with a distinct assignment, the complete ordered owner request and corrections, prior confirmed discussion, relevant source ledger and role/output contract. The server renders behavioral prompts from the exact runtime-instructions Markdown snapshot accepted with the run. Head records the requested outputs, gives each selected specialist a distinct, substantive task, and the successfully returned task text is committed and delivered verbatim to that recipient. The server never rejects a task for wording, truncates it or substitutes a generic fallback. Each specialist's independent initial position is addressed to Critic; every selected specialist receives at least one direct Critic challenge and reply. Head decides whether to continue after each whole-team review, within the selected depth ceiling. Each specialist then sends a final position to Head; Critic reviews all final positions together; only then does Head address the owner. No specialist-to-specialist routing, extra AI server orchestrator or direct Head-answer route is permitted. | UC-002 |
| FR-02.5 | A material Critic objection receives a relevant consultant response or revision; warranted agreement is permitted immediately. For an unacceptable circular, fabricated, irrelevant or unsupported contribution, Critic identifies the exact defect and issues a targeted stop-and-rework directive. The consultant must materially correct the answer, give a specific evidence-based objection or acknowledge missing evidence. Head sees and arbitrates the response; repeated unproductive exchanges do not fill a round quota. | UC-002 |
...
| FR-03.2 | The first substantive request sets English or Ukrainian session language; an explicit supported-language change wins and role names remain English. Russian and Belarusian language and terminology are rejected in owner input, generated content and saved source metadata. Valid Ukrainian shared words alone are not prohibited-language evidence. A generated draft that violates this policy, or contains provider tool invocation/transcript material, is never stored or rendered; the same role/task receives one bounded compliant-replacement attempt before the consultation can fail. | UC-002 |
...
| FR-03.4 | During active work hide the unavailable composer and keep one deliberately placed square Stop control away from the former Send position and routine reading taps. It remains labelled and keyboard/touch reachable, gives pending/failure feedback, and prevents late work from becoming a newly visible result. Stop, completion or failure restores the composer without losing an unsent page-memory draft. | UC-003 |
| FR-03.5 | Continue resumes a stopped or bounded consultation with its accepted context; Retry resumes a failed one at the missing reply with its accepted settings and instructions. Confirmed messages remain unchanged and every required specialist/Critic contribution still precedes Head synthesis. | UC-003 |
...
| FR-05.3 | Settings provides specialist count 1/2/3/5/Auto, discussion-depth ceiling 1/3/5/Auto and incoming-message sound Knock/Chime/Ripple/Off with direct preview. Specialist count excludes Head and Critic. Every selected specialist receives at least one complete Critic challenge and reply; Head may close after that whole-team review or request another targeted round up to the selected ceiling. Auto permits at most ten such rounds per specialist. The ceiling is never a quota for filler. All selected specialists must contribute a closing position before Critic evaluates them together; Head synthesis requires those committed messages and remains provisional when disagreement persists. Final positions and closing review do not count as review rounds. Load the saved sound preference on authenticated entry without requiring Settings, including Off. Use the same sound for Preview and newly confirmed agent-message alerts; initial history/reload, repeated polling, owner messages and System notices do not trigger alerts. Local browser WAV playback never requests notification permission. A browser playback block is reported with an explicit Enable sound action and a visual equivalent; do not claim delivery while the browser, operating system or device suppresses sound. The 540,000 ms provider budget remains separate. | UC-005 |
...
| NFR-01.3 | Enforce the saved discussion-depth ceiling: Head decides after each complete team review whether another targeted round would add value, with at most 1, 3 or 5 rounds as selected, or 10 in Auto. A complete first challenge/reply for every selected specialist precedes closure. No setting requires filler or ceremonial rounds. Retain the ten-minute continuation boundary. | UC-003 |
...
| AC-002 — Useful exchange (UC-002) | Head inventories every owner-requested output, chooses the actual team, commits distinct verbatim recipient-specific assignments and decides whether another useful review is needed. Every selected specialist and Critic has a separate invocation, one direct challenge/reply at minimum, a final position and joint closing review before Head concludes. A circular or fabricated answer elicits a targeted Critic stop-and-rework and a material correction, grounded objection or explicit evidence gap. No single completion can pass as several agents; no task is replaced by a generic fallback or final deliverable silently omitted. |
