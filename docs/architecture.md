...
Image intake accepts only the authenticated owner’s JPEG, PNG or WebP up to 8 MiB, enforces the byte cap before storage and verifies the binary signature instead of trusting MIME. Store an opaque encrypted original with an internal name; never execute, transform or thumbnail it server-side. Protected retrieval uses the validated type, attachment disposition and `nosniff`. PDF, SVG, audio, video, archives, truncation and mismatch fail before acceptance. The self-generated-image statement is not technical provenance proof.
...
| NFR-12.2 | 8 MiB image/signature checks, encrypted opaque storage and protected attachment retrieval; browser speech creates no app audio endpoint. |
