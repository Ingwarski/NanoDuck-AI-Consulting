...
4. **Build minimum sufficient context from the canonical chat.** Extract a context helper from coordinator/provider prompt construction. Include the complete current accepted owner message once, own task/role, and referenced evidence/current results. Retain attachment capability disclosure, required guidance and exact language/model choices. Use internal message boundaries, not a duplicate request document or Claude file tools. Preserve full chat separately from per-call input.
...
## Впровадження режиму читання — 2026-09-26

Явний запит «імплементуй» авторизує погоджену зміну S-02: `public/index.html`, `public/styles.css`, `src/client/app.js`; браузерні перевірки у `scripts/browser-reading-mode.mjs` та адаптація чинних перевірок навігації. Інтерфейс клієнтський, серверних змін немає. Прийняття: чернетка й вибраний розділ збережені, Stop/Retry доступні, завершений висновок читається без великого поля вводу. Electric A v8 з погодженим scoped override; прототипи не копіюються. Формальна готовність релізу not_evaluated.

## Isolated request and cache economy — 2026-09-26

Unit U-10: isolated request context and provider cache economy (full-stack). Sources FR-02.12/FR-02.15, UC-002/UC-003/UC-005; depends on U-09. Implement immutable request binding and source scoping; shared stable prompt builder; compact Head control context; targeted public research; minimal Codex base instructions and request workspace lifecycle; truthful stage telemetry/UI. Preserve full owner input, current results, exact models and selected depth. Destinations: src/server/{store,local-state,consultation,consultation-parallel,provider-context,codex-provider,claude-provider,providers,usage}.mjs, src/client/app.js, public/{index.html,styles.css}. Verification: context-economy/provider/usage regressions, full check, three-engine Usage browser checks and bounded synthetic live benchmark. Existing QA mappings and Electric A v8 apply; no prototype reuse. Explicit current user request authorizes branch implementation, validation, merge and push to main; no sister-repo changes. Portable instructions are in docs/context-cache-isolation-port.md. Formal release readiness remains not_evaluated.
...
This is a boundary-visibility correction to U-11, explicitly authorized by the current user message. Destinations: src/client/app.js; browser-reading-mode.mjs verifies visibility. No prototype promotion.

## U-12 research and evidence economy — 2026-09-26

U-12 — research execution and request-scoped evidence economy. Explicit current user request: form and implement this unit. Depends on U-09/U-10; FR-02.12/FR-02.15/FR-04.1/FR-04.2, UC-002/UC-003/UC-007. Sequence: (1) correct scripts/measure-consultation-workload.mjs to use buildProviderContext and add research scenarios; (2) preserve distinct claims in both adapters; (3) add content-free research diagnostics and a bounded focused/combined benchmark; (4) move evidence into shared prefixes and checkpoint validated follow-up queries; (5) implement stable validated evidence references with visible-link resolution; (6) Head-directed recipients and explicit reuse/fresh decisions; (7) recovery, isolation, provider, browser and SDD checks; (8) portable sister-repo prompt, commit/push branch and merge main. Backend owns evidence/checkpoints/contracts; provider adapters own extraction/telemetry; client/export consume resolved messages without new interfaces. Destinations src/server/{research-evidence,consultation-parallel,parallel-contract,codex-provider,claude-provider,usage}.mjs and scripts/tests. Preserve approved visual baseline, no prototype reuse. Do not interrupt active work; activate after it ends. Formal release readiness not_evaluated.

U-12 implementation and scoped verification complete; see [verification](../forge/runs/research-economy-20260926/verification.json) and [portable implementation prompt](research-evidence-economy-port.md). Production settings and visual foundation are preserved. Activate only without active consultations; whole-product release readiness remains not_evaluated.
...
U-12 Critic cache/usage follow-up implementation and scoped verification complete; see [verification](../forge/runs/critic-cache-usage-20260926/verification.json) and [portable implementation prompt](research-evidence-economy-port.md). Production settings and visual foundation are preserved. Activate only without active consultations; whole-product release readiness remains not_evaluated.
...
U-12 upstream accounting follow-up implemented and verified. Pinned experimental raw completion metadata is numerically allowlisted, deduplicated per response and reconciled with totals. Existing Usage shows accounting provenance. [Evidence](../forge/runs/upstream-usage-20260926/verification.json); [portable prompt](research-evidence-economy-port.md). Activate only without active consultations; release_readiness remains not_evaluated.

## U-13 actionable usage dashboard — 2026-09-26

U-13 — actionable usage dashboard. User authorization: “implement all of it.” Implement attribution and aggregation, native account-limit read, independent authenticated endpoint, provider-first responsive UI, request/participant/activity rankings, repeat-work union and timed attempt details. Verify focused tests, full checks and three browser engines; publish this repository only. Do not interrupt active consultations. No claim of exact subscription deductions or physical cache writes.

U-13 implemented: provider summaries, independent account allowance, visible cache breakdown, request/participant/activity grouping, repeat-work union, coverage states and timed/filterable attempts. [Verification](../forge/runs/usage-dashboard-20260926/verification.json); [sister-repository instructions](usage-dashboard-port.md). Historical attribution remains unknown; Claude account allowance opens its official usage page because the current noninteractive result lacks percentage windows. Preserve release_readiness not_evaluated.
...
U-13 reasoning attribution follow-up implemented. Adapters persist requested effort, summaries separate model configurations, and Usage labels models and calls. Conservative saved-run recovery covers matching single-request historical records. Verification: [evidence](../forge/runs/usage-reasoning-20260926/verification.json). Sister-port prompt updated. Saved-run comparison is observational, not a causal quality benchmark.
...
Glass rim refinement implemented with hollow masked pseudo-elements, polished highlights and a shaded optical edge. The centre stays clear; all decoration ignores pointer events. [Verification](../forge/runs/glass-rim-20260926/verification.json). No restart or active-page reload.
...
Actual navbar refraction renderer implemented, superseding the rejected CSS rim/bevel treatment. Local disposable canvas samples a narrow app-content strip and displaces edge pixels; centre and controls remain native. Privacy clearing is integrated. [Verification](../forge/runs/navbar-refraction-20260926/verification.json). No arbitrary-media or native Apple-renderer claim.
