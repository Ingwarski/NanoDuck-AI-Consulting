...
14 September 2026 · Status: validated specification; **awaiting-implementation-prompt**. No implementation unit has started. Design approval and continuation of SDD authorize this plan, not production changes.
...
The inspected base is `f99744d603b4e27cb7e89eccde2f12b9652e8a01`. `package.json` starts only `scripts/serve-prototype.mjs` and runs `scripts/check-artifacts.mjs`; there are no production `src/`, `public/` or `tests/` trees. Frozen A v8 contains five presentation files. `prototype/` and `docs/design.md` are rejected history, never production seeds.
...
**Owner/layer:** authorized implementation operator, infrastructure / integration. **Depends on:** None; first work after a separate implementation prompt.
...
**Work:** Obtain a fresh content-free saved/effective settings snapshot, including inactive Claude preferences. Independently verify Head/specialists Codex gpt-6-astra xhigh and Critic Codex gpt-6-astra ultra, Balanced speed, Codex 0.153.1 and Claude Code 2.1.258. Preserve saved values; never substitute, downgrade, buy credits or enable a paid fallback. Verify catalog/usage/renewal capabilities only for the selected route; do not probe unused Claude. Exercise a short allowed subscription call and restricted research capability after authorization. Verify browser-produced audio MIME types against the existing subscription transcription adapter on the supported browsers. Inspect the exact GoDaddy app runtime/subprocess, idle/background lifecycle, restart/redeploy storage, stream buffering, TLS, environment and logs without resetting data. Resolve upload/audio/research limits, safe parser/malware mechanism, session concurrency/federated termination, separated key handling, backup retention/RPO/RTO and operator procedures through architecture before dependent tests. Use isolated fixtures and keep secrets/content out of Git.
...
**Risk/stop condition:** Host lifecycle or subscription transcription incompatibility. Stop the affected runtime branch before committing its design; other independent UI work may proceed.
...
**Owner/layer:** authorized implementation operator, full-stack. **Depends on:** U-01 audio/parser capability; U-02 protected storage; U-03 composer; U-04 accepted-input contract.
...
**Work:** Implement Start-gated microphone permission and verified MIME capture, Stop-to-transcribe, Cancel-to-erase, background interruption and release of every media track. Use only the verified subscription transcription path. Return editable text to the existing draft at a clear append boundary; no automatic Send. Distinguish permission, hardware, interruption and transcription errors, preserving typing and explicit retry. Validate supported raster/PDF/audio content, sizes and safe parser/malware outcomes before processing. Store accepted conversation attachments encrypted and non-executable with ownership; temporary audio remains separate and is erased after completion/cancel/failure.

**Acceptance:** On real supported devices, a spoken request becomes editable unsent text; Cancel/background/failure leaves no active hidden capture or reusable temporary audio. Invalid or unsafe uploads fail without losing the typed draft. Typing fallback remains usable but cannot waive required voice acceptance.
...
**Risk/stop condition:** Browser capture success does not prove provider decoding or privacy cleanup. Exercise the full capture-to-transcript seam and inspect media/resource release.
...
| C-04 role execution, U-04 → U-05/U-06 provider operations | Separate role context and assignment, exact tuple, scoped operation/tool permissions, bounded cancellation and categorized failure. Research/transcription do not acquire general shell/filesystem/computer permissions. | QA-R05–QA-R10, QA-R17–QA-R19, QA-R28–QA-R32, QA-R51; compare to exact clause mapping below. |
...
| C-06 inputs, U-06 → U-04/U-03/U-07 | Owned validated attachment references and editable unsent transcript; transient audio never a saved conversation attachment. Explicit Send alone creates an accepted event. | FR-02.2/FR-07.1–FR-07.5 exact QA mappings below, QA-R50, QA-R55, QA-U01. |
...
This document executes no implementation, promotion, provider call, microphone capture, database reset or deployment. The future product excludes messengers, public/multiuser SaaS, payments, extra hosts, paid fallback, automatic external business actions, required notifications, native apps and arbitrary uploads. Unchosen palette alternatives are historical design references. Do not alter any unrelated GoDaddy resource. Any database reset remains conditional on proven exclusive ownership and recovery/cutover evidence.

## Open Questions

No further aesthetic or product-intent choice is needed to finish this plan. U-01 owns the unresolved current saved settings/entitlements, exact browser audio/transcription combinations, host lifecycle/streaming/storage and configuration/security parameters described in architecture. The product owner resolves any material change of behavior, provider, budget or hosting scope; the implementation operator returns ordinary evidenced mechanism choices to architecture, then revalidates affected downstream artifacts. U-08 requires actual operational ownership, representative-owner availability and exact database/table identity before release. These questions do not make unrun checks pass and cannot be silently waived.

## Handoff and implementation boundary

The approved design, reconciled architecture/DoD/QA and this plan are ready for a later explicit implementation request. Orchestrator state: `awaiting-implementation-prompt`. The gate must bind this plan's current hash and `nanoduck-electric-a-v8-20260914` and contain no implementation prompt receipt until a later explicit user request arrives after this completed-plan pause. “Design accepted”, “continue”, plan completion, Git commit/push and historical authorization do not release Phase 3. Next authorized work in this turn ends with checking and committing these specification artifacts.
