...
**Acceptance:** Separate provider/context evidence proves every accepted question follows the selected-team discussion, including a simple question. Head gives no owner-facing content before an evidence-consistent final conclusion of at most three actions. Crash-after-acceptance, duplicate retry, restart and Stop/late-result races produce exactly one confirmed visible step; a repeated provider call after an uncommitted result is recorded honestly. New cannot bypass one active run.
