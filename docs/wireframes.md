...
The discussion is the primary reading surface. Navigation stays available, then current topic/context, complete messages and a reachable composer when input is available. Active work hides that composer and reserves a separate Porcelain Stop control in the upper-right Discussion header, away from the former Send position and message reading targets. Group by purpose; do not wrap every sentence in nested cards. The owner must be able to inspect the whole record, steer and use voice with touch or keyboard. Labels persist when fields have values.
...
|---|---|---|---|
| S-01: enter private work. J-01, JOB-003, UC-001 | Product identity → concise purpose → labelled local-password entry → processing explanation once per new authenticated session | Sign in / cancel; explicit consent then open workspace | Password uses a native masked field with password-manager/paste support; it never requests provider credentials. Each new authenticated session starts with unchecked consent; denied/expired/attempt-limit states replace entry feedback without showing history. |
| S-02: describe, discuss and control. J-02/J-03/J-05, JOB-001/JOB-002, UC-002/UC-003 | Persistent navigation → workspace action row with New → Discussion eyebrow and compact status, with no active-conversation heading → local Discussion/Outcome/Sources views → chronological role/recipient/time messages → visible thought-bubble state during work → composer only when input is available | Send; separated Porcelain Stop in the upper-right Discussion header during work / Continue when stopped / Retry after failure; New | Labelled text field, image-attachment and icon-only microphone button with an accessible Voice input name; a high-contrast, non-shrinking Send button; no automatic send. Enter submits and Shift+Enter makes a line break. The picker accepts JPEG, PNG and WebP only. Empty state puts composer first. Clarification adds one question and an answer field. Long content wraps without truncating submitted text. During active work hide the text/attachment/voice/Send panel as one unit; preserve unsent page-memory input and restore it after Stop, completion or failure. Do not move Stop into the former Send position. Stop remains available across the local consultation views, with pending/error feedback. To add context, Stop then explicitly Send; Continue resumes accepted work. A browser refresh remains in this tab and, after the protected record reloads, returns to its selected local tab and reading position. The thought bubble uses text as well as motion; new agent messages retain a visual and screen-reader signal even when sound is off or blocked. |
...
| S-04: change future settings. J-07, JOB-003/JOB-005, UC-005 | Heading → Head/specialists fieldset → Critic fieldset → number of specialists → discussion depth → message-sound select/preview → runtime-instructions and fixed-name guidance document selector/current revision → saved-version list → actual usage → account/security actions | Save settings / Preview sound / Save runtime instructions / Review version / Restore reviewed version / Cancel, Logoff | Labelled native model and reasoning selects in each fieldset; Critic has provider select. Specialist count excludes Head and Critic. Depth labels explain fixed 1/3/5 team review rounds and Head-controlled Auto up to ten; only affected consultants need correction replies. No mandatory closing speeches are implied. Sound offers Knock, Chime, Ripple or Off; Preview occurs only after the owner activates it. Saved sound applies without visiting this surface; a playback block leaves visual feedback intact and adds a contextual Enable sound action in Discussion or Settings. The fixed-name guidance selector uses the same editor and version controls; guidance documents follow their 64 KiB rule. The separate runtime-contract Markdown document identifies its encrypted current revision and required headings/placeholders. The version list exposes metadata only until the owner opens a read-only review dialog; Restore creates a fresh current revision. Invalid combinations, Markdown or stale revision show the specific requirement, retain the unsaved edit and do not change the saved future-run value. Unavailable catalog preserves the saved selection. A selected Claude authorization failure adds concise local sign-in guidance and Check connection beside its status; no credential fields. Pending refresh prevents repeat checks and retains every unsaved form value, with truthful ready/auth/quota/unavailable feedback. |
| S-05: dictate with control. J-02, JOB-006, UC-006 | Labelled voice surface → browser-recognition disclosure and state → timer paired with status → transcript field when ready | Start/Stop/Use transcript / Cancel, Retry, Type instead | No recognition before Start; unsupported browsers retain Type instead. The dialog identifies the browser recognition-service boundary; NanoDuck receives no audio. Use transcript returns editable text to S-02 without sending. Retain a preexisting draft; insertion appends at a clear boundary rather than silently replacing it. |
...
|---|---|
| Signed-out, consent, denied, expired | Explain entry/expiry; retain server history privately; enter the local password or cancel; retry after the stated limit if needed; private workspace appears only after a valid session. |
...
| Active and stopped | Pair ongoing state with a labelled thought bubble and separated Porcelain Stop; hide the unavailable composer, retain confirmed messages and unsent page-memory draft; Stop restores input for explicit Send or Continue; return or New; no new late result appears after Stop. A refused Stop retains the active controls and explains the failed request. A single policy-rejected provider draft is withheld and replaced before this recovery state is used. |
...
| Settings saved/edited/invalid/catalog-unavailable | Show selected values, encrypted runtime-instructions current revision, saved-version metadata and precise validity; retain saved/current-run settings and unsaved document text; choose a compatible value, restore a required placeholder, review a saved version before explicit restore or cancel; retry catalog only when needed; saved-for-next-run feedback never rewrites active work. |
| Session control | Use visible Logoff to end the current session and clear private browser content while preserving server conversations. Local operator password reset invalidates all sessions. No current/other-session list, selective revocation control or extra confirmation screen is introduced. |
