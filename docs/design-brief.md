...
The composer offers text, owner-generated image attachment and voice without hiding voice in Settings. The approved presentation already includes this attachment control, so the JPEG/PNG/WebP-only runtime rule does not alter the Electric A v8 visual baseline. An active consultation replaces ordinary progress with a compact thought bubble, three restrained dots and explicit text; it always exposes Stop within reach. The design never depends on the motion or selected notification sound to convey the incoming-message state. There is no oversized current-conversation heading: the Discussion eyebrow and compact state line remain during work, and a concise history title is created only after final synthesis. A browser-recognized transcript never silently becomes a sent message; Use transcript inserts editable text and explicit Send remains separate. Preserve existing typed text during voice failure/cancel. Browser-native recognition and its disclosure are runtime behavior specified by the PRD; they do not alter the approved Electric A v8 visual baseline.
