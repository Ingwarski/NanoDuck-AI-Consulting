...
| Conventional floating navigation | Discussion, Conversations and Settings on desktop; labelled hamburger on mobile. Both include the session action Login or Logoff, using the existing primary blue/white button treatment and visible keyboard focus. Switch to mobile navigation before the additional action crowds the brand; retain at least 44px touch height. New is outside these menus, in the workspace action row and Conversations. |
