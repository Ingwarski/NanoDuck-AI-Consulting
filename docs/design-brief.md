...
| Settings controls | Independent model/reasoning controls, optional Critic provider, a native message-sound select with owner-activated Preview, and a visible full-height editable runtime-instructions Markdown document from encrypted database storage, with current revision/status and a compact saved-version list. Sound options are Knock, Chime, Ripple and Off; visual feedback remains when sound is off or blocked. Review opens a read-only dialog; Restore is explicit and creates a new current revision. Each save or restore affects only the next run; malformed required headings/placeholders or a stale revision show a specific local error. |
