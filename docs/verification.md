...
Formal QA remains **82 prepared / not_run**; all six release gates remain unevaluated. The owner later explicitly authorized Phase 3. The local browser-native voice-input unit now opens the disclosed dialog, starts recognition only from its labelled control, returns editable text on Stop and never creates an application audio upload or transcription request. In this session, the local workspace walkthrough verified the dialog and Cancel path without enabling the microphone; the owner separately confirmed a Ukrainian result in Safari and Chrome. A separate read-only GoDaddy audit identified the named app's shared Preview/Published managed-database table inventory without opening rows, secrets or exports. U-08 has now added [local integration and cutover-preparation evidence](../forge/runs/U-08/phase3-local-integration-preparation-20260914/verification.json) and its [non-deploying cutover package](../forge/runs/U-08/phase3-local-integration-preparation-20260914/cutover-package.md): local routes, controlled orchestration, settings persistence and narrow-layout behavior were observed with a memory store and fake provider, while the test suite passed. A subsequent synthetic Ukrainian direct-question smoke test used the actual local Codex app-server and returned Ukrainian Head Consultant output; this proves the fixture route has been replaced for local text testing, not research, multi-agent, voice, production or release behavior. The current U-04 correction replaces the prior malformed discussion routing with visible Head-to-specialist tasks, independent specialist-to-Critic positions, Critic-to-specialist exchanges and one final Head synthesis. Its [recipient-topology correction receipt](../forge/runs/U-04/phase3-recipient-topology-correction-20260914/verification.json) records deterministic consultation/HTTP checks and one synthetic no-sensitive local browser run against the actual provider. That run displayed the required two-specialist sequence, a Critic exchange and one final Head synthesis, with no pre-conclusion Head advice or specialist-to-specialist route. This is one observed prompt-compliance sample, not research, voice, production or release evidence. The evidence still leaves supported-device/assistive checks, representative-owner tasks, target ownership/restore and every formal release gate unrun. No GoDaddy resource was changed.
