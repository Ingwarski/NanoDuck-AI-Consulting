...
Read-only inspection on 15 September confirmed the named **Personal AI Consulting Group** app (`wy2v0putg6`) as the only target: Node 22. Its GitHub source was then switched to `Ingwarski/personal-ai-consulting-web` on `main`; Preview is still unsynchronized. GoDaddy injects the target database as `DB_*` and provides encrypted environment secrets rather than mounted files, so the app forms a certificate-verified MySQL connection from the managed values and supports a plain or gzip/base64 Codex auth secret that exists only inside a private, removed-after-use app-server directory. GoDaddy rejected the plain instruction bootstrap and the plain Codex auth payload as too long; the local configuration now accepts tested gzip/base64 alternatives, each mutually exclusive with its plain form. Production startup applies its idempotent migration before serving. Local tests exercise the compatibility boundaries; the [U-08 host-compatibility receipt](../forge/runs/U-08/godaddy-host-compatibility-20260915/verification.json) records the pre-switch source bindings. Hosted startup, MySQL, provider, Google sign-in and publication remain unverified.
