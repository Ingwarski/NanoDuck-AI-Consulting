...
The local server was restarted only after confirming no active consultation. An in-memory comparison confirmed unchanged conversation, message, run, settings and instruction content. HTTPS health and current asset bytes passed on loopback and a private LAN interface with the existing CA verified. This same-computer LAN request does not replace a second physical device test. No password, trust store or provider credential was changed. Cross-platform CI is recorded separately when available.
