...
The named **Personal AI Consulting Group** app (`wy2v0putg6`) remains the only target. Preview deployed `Ingwarski/personal-ai-consulting-web` at `c61fbb0`; its infrastructure and site status report OK. GoDaddy injects the target database as `DB_*` and provides encrypted environment secrets rather than mounted files, so the app forms a certificate-verified MySQL connection from the managed values and supports a plain or gzip/base64 Codex auth secret that exists only inside a private, removed-after-use app-server directory. GoDaddy rejected the plain instruction bootstrap and the plain Codex auth payload as too long; the deployed configuration uses the tested gzip/base64 alternatives. Preview created ten `nanoduck_*` tables and persisted an encrypted instruction record. The initial consent screen was reached through the host's development sign-in path, so it does not prove Google authentication. `NANODUCK_RUNTIME_MODE=production` is saved for Preview and the server change must deploy before the Google route can be checked. [Pre-switch host compatibility](../forge/runs/U-08/godaddy-host-compatibility-20260915/verification.json) and the [Preview activation receipt](../forge/runs/U-08/godaddy-preview-activation-20260915/verification.json) retain their separate scopes. Provider preflight/execution, consent, a real consultation, Ukrainian voice, restart recovery, publication remain unverified.
