...
| ST-09 | S-02 | `active` — visible thought bubble/text, reachable Stop, and no in-progress title | `#discussion` · UC-002, UC-003 |
...
J-03 clarification and substantive exchange remain on S-02; no procedural stage screen is added. Its active state shows the quiet thought-bubble text and may play the selected sound only after a newly confirmed agent message; visual feedback is always present. J-04 moves to S-06 and back to the originating claim. A blocked generated draft is not a user-visible `system-error` on its first policy rejection: the same role/task makes one bounded replacement attempt. J-05 Stop/Continue preserves the same record, while New ends or pauses active work before establishing another. Offline, quota, provider-auth and persistent system/research failure keep distinct recovery transitions. App expiry at the PRD 24-hour boundary goes through S-01; explicit sign-out, revocation and security invalidation remain earlier exits.
