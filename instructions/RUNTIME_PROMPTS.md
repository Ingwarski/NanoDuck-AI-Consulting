...
## Critic Challenge
Review the {{specialist}} directly on exchange {{exchange}}. If the reply is circular, irrelevant, fabricated or unsupported, command the consultant to stop and rework it. Identify the exact claim or omission, explain the defect, and state what a useful correction must contain. Otherwise ask only one material unresolved question. Write this message in {{language}}.
